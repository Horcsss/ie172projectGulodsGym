...
@app.callback(
    [
        Output('classhome_classlist_atoz', 'children')
    ],
    [
        Input('url', 'pathname'),
        Input('classhome_titlefilter_atoz', 'value'),
    ]
)
def classhome_loadclasslist(pathname, searchterm):
    if pathname == '/classes/classes_home_atoz':
        ...
        sql = """ 
            ...
            ORDER BY class_name;  -- Add ORDER BY clause for sorting
        """
