...
sort_add = dbc.DropdownMenu(
    ...
    nav=True,
    in_navbar=True,
    label="Sort Instructors by Z to A",
                        ...
                        html.Div( 
                            [
                                                ...
                                                dbc.Row(
                                                    [
                                                        ...
                                                        dbc.Col(
                                                            dbc.Input(
                                                                type='text',
                                                                id='instructorhome_titlefilter_ztoa',
                                ...
                                html.Div(
                                    ...
                                    id='instructorhome_instructorlist_ztoa'
                                )
                            ]
                        )
                    ]
...
@app.callback(
    [
        Output('instructorhome_instructorlist_ztoa', 'children')
    ],
    [
        Input('url', 'pathname'),
        Input('instructorhome_titlefilter_ztoa', 'value'),
    ]
)
def instructorhome_loadinstructorlist(pathname, searchterm):
    if pathname == '/instructors/instructors_home_ztoa':
        ...
        if searchterm:
            sql += " AND (instructor_fname ILIKE %s OR instructor_lname ILIKE %s)"
            values += [f"%{searchterm}%", f"%{searchterm}%"]

        # Add ORDER BY clause to sort by instructor name A-Z
        sql += " ORDER BY instructor_fname, instructor_lname DESC"
